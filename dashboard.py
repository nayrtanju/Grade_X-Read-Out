...
def action_items(overview: pd.DataFrame, details: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    persistent = pd.to_numeric(out.get("Persistent DTC Count", pd.Series(index=out.index, dtype=float)), errors="coerce").fillna(0)
    dtc_codes = out.get("DTC Codes", pd.Series(index=out.index, dtype=str)).fillna("").astype(str)
    diagnostic_mask = persistent > 0
    out.loc[diagnostic_mask, "Priority"] = out.loc[diagnostic_mask, "Priority"].clip(upper=2)
    out.loc[diagnostic_mask, "Recommended Action"] = (
        "Investigate persistent DTC(s): " + dtc_codes[diagnostic_mask]
    )
    out["Diagnostic Action"] = diagnostic_mask
    ...
    open_mask = (out.get("Status", pd.Series(index=out.index, dtype=str)) != "COMPLIANT") | diagnostic_mask
    return out[open_mask][available].sort_values(
        ["Priority", "Persistent DTC Count", "Confidence %"], ascending=[True, False, False]
    ).reset_index(drop=True)
