...
with tabs[0]:
    if filtered_overview.empty:
        ...
        with right_chart:
            ...
            st.bar_chart(confidence_df.set_index("Confidence Band")["Count"], use_container_width=True)
            st.dataframe(confidence_df, use_container_width=True, hide_index=True)

        st.subheader(t["vehicle_summary"])
        st.dataframe(vehicle_summary(filtered_overview), use_container_width=True, hide_index=True)
        ...
        if actions.empty:
            ...
            st.dataframe(actions.style.map(status_style, subset=["Status"]), use_container_width=True, hide_index=True)
...
with tabs[3]:
    st.dataframe(filtered_details.style.map(status_style, subset=["Status"]), use_container_width=True, hide_index=True)
with tabs[4]:
